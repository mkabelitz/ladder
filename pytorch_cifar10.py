...
cifar10_tr_dataset = datasets.CIFAR10('./torchvision_data', train=True, download=True, transform=transform)
cifar10_te_dataset = datasets.CIFAR10('./torchvision_data', train=False, transform=transform)

if args.labeled_samples:
    balanced_index_set = []
    class_counts = [0] * 10
    overall_count = 0
    for i in range(cifar10_tr_dataset.__len__()):
        if overall_count == args.labeled_samples:
            break
        cur_class = cifar10_tr_dataset.__getitem__(i)[1]
        if class_counts[cur_class] < args.labeled_samples / 10:
            balanced_index_set.append(i)
            class_counts[cur_class] += 1
            overall_count += 1
    train_loader = torch.utils.data.DataLoader(cifar10_tr_dataset,
...
unlabeled_loader = torch.utils.data.DataLoader(cifar10_tr_dataset, batch_size=args.batch_size, shuffle=True, **kwargs)
test_loader = torch.utils.data.DataLoader(cifar10_te_dataset, batch_size=args.test_batch_size, shuffle=True, **kwargs)

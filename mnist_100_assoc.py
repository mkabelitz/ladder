...
def main(_):
    ...
    loss_sup = u.get_supervised_loss(logits=logits_tr, labels=labels_tr_batch)
    loss_aba, loss_visit = add_semisup_loss(emb_tr, emb_ul, labels_tr)
    loss_tr = loss_sup + loss_aba + loss_visit
    ...
    with tf.Session() as sess:
        ...
        for i in tqdm(range(FLAGS.num_iters)):
            _, cur_loss_tr, cur_acc_tr, cur_loss_sup, cur_loss_aba, cur_loss_visit = sess.run([train_op, loss_tr, acc_tr, loss_sup, loss_aba, loss_visit])
